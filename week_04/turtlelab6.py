...
main_globals = vars(sys.modules["__main__"])
...
def verify_turtle():
    ...
    """
    try:
        submission_turtle = main_globals["turtle"]
    except KeyError:
...
def submitted_byte_count():
    ...
    """
    try:
        submitted_file = main_globals['__file__']
...
class Task:
    """
    TASK: help guide Turtle back home while stop to visit Luigi and
    Mozart Luigi's, Mozart's, and Turtle's home will randomly appear
    ANYWHERE on the map.
    """
    def start(self):
        stage.reset()
        ...
        if INTERACTIVE:
            ...
            shuffle(locations)
            (luigi_x,luigi_y),(mozart_x,mozart_y) = locations
        else:
            luigi_x,luigi_y = int(input()),int(input())
            mozart_x,mozart_y = int(input()),int(input())
            home_x,home_y = int(input()),int(input())

        self.home = Home(home_x,home_y,32,32,IMG_HOME,"home")
        self.luigi = Home(luigi_x,luigi_y,32,32,IMG_HOME,"luigi")
        self.mozart = Home(mozart_x,mozart_y,32,32,IMG_HOME,"mozart")
        stage.add_object(self.home)
        stage.add_object(self.luigi)
        stage.add_object(self.mozart)
        stage.add_object(Road(0,0,luigi_x,luigi_y,5,"yellow"))
        stage.add_object(Road(0,0,mozart_x,mozart_y,5,"yellow"))
        stage.add_object(Road(luigi_x,luigi_y,mozart_x,mozart_y,5,"yellow"))
        stage.add_object(Road(home_x,home_y,luigi_x,luigi_y,5,"yellow"))
        stage.add_object(Road(home_x,home_y,mozart_x,mozart_y,5,"yellow"))
        if INTERACTIVE:
            print(dedent(self.__doc__).strip())
            print(f"Luigi appears at ({self.luigi.x:.2f},{self.luigi.y:.2f})")
        ...
        return stage.turtle,self.home,self.luigi,self.mozart

    def shortest_path(self):
        dist1 = ( distance(0,0,self.luigi.x,self.luigi.y) +
                  distance(self.luigi.x,self.luigi.y,self.mozart.x,self.mozart.y) +
                  distance(self.mozart.x,self.mozart.y,self.home.x,self.home.y) )
        dist2 = ( distance(0,0,self.mozart.x,self.mozart.y) +
                  distance(self.mozart.x,self.mozart.y,self.luigi.x,self.luigi.y) +
                  distance(self.luigi.x,self.luigi.y,self.home.x,self.home.y) )
    ...
    def done(self):
        ...
        if INTERACTIVE:
            ...
            print("Seed =",14)
        print("Check if Turtle stopped at Luigi's: ", end="")
        if any(self.luigi.contains(stop) for stop in stage.stops):
            print("PASSED")
        else:
            print("FAILED")
...
task = Task()
turtle,home,luigi,mozart = task.start()
mod.check = task.done
mod.turtle = turtle
mod.home = home
mod.luigi = luigi
